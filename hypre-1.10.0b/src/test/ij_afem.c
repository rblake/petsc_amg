...
*/
   ...
   while ( (arg_index < argc) && (!print_usage) )
   {
      ...
      else if ( strcmp(argv[arg_index], "-afem") == 0 )
      {
         arg_index++;
         build_matrix_type      = 6;
         build_matrix_arg_index = arg_index;
	 ...
	 if ( strcmp(argv[arg_index], "none") == 0 ) {
      ...
      else if ( strcmp(argv[arg_index], "-rhszero-randguess") == 0 )
      {
         arg_index++;
         build_rhs_type      = 6;
         build_rhs_arg_index = arg_index;
      ...
      else if ( strcmp(argv[arg_index], "-cljpc") == 0)
      {
	 arg_index++;
	 coarsen_type = 20;
      }
      else if ( strcmp(argv[arg_index], "-bsis") == 0)
      {
	 arg_index++;
	 coarsen_type = 120;
      }
      else if ( strcmp(argv[arg_index], "-pmisc1") == 0)
      {
	 arg_index++;
	 coarsen_type = 21;
      }
      else if ( strcmp(argv[arg_index], "-pmisc2") == 0)
      {
	 arg_index++;
	 coarsen_type = 22;
      }
      ...
      else if ( strcmp(argv[arg_index], "-cr-cljpc") == 0 )
      {
         arg_index++;
         coarsen_type      = 30;
      }    
      else if ( strcmp(argv[arg_index], "-cr-pmisc1") == 0 )
      {
         arg_index++;
         coarsen_type      = 31;
      }    
      else if ( strcmp(argv[arg_index], "-cr-cljp") == 0 )
      {
         arg_index++;
         coarsen_type      = 32;
      }    
      else if ( strcmp(argv[arg_index], "-cr-pmis") == 0 )
      {
         arg_index++;
         coarsen_type      = 33;
      }    
      ...
      {
         arg_index++;
         coarse_relax_default = atoi(argv[arg_index++]);
      }
      ...
      else if ( strcmp(argv[arg_index], "-mxits") == 0 )
      {
         arg_index++;
   ...
   if ( (solver_id == 0 || solver_id == 101) && lobpcgFlag )
     solver_id = 1;
   ...
   if (solver_id == 0 || solver_id == 1 || solver_id == 3 || solver_id == 5
	|| solver_id == 9 || solver_id == 13 || solver_id == 14
 	|| solver_id == 15 || solver_id == 20 || solver_id == 101)
   {
   ...
   /* coarsest grid */
   num_grid_sweeps[3] = 1;
   grid_relax_type[3] = coarse_relax_default;
   ...
   while (arg_index < argc)
   {
      ...
      else if ( strcmp(argv[arg_index], "-w") == 0 )
      {
		...
		|| solver_id == 15 || solver_id == 9 || solver_id == 20 || solver_id == 101)
        {
   ...
   if ( (print_usage) && (myid == 0) )
   {
      ...
      printf("  -rhszero-randguess     : rhs is zero vector, initial guess is random with\n");
      printf("                         :  compenents in range 0 - 1\n");
      printf("\n");
      ...
      printf("  -cljpc                : CLJP-c coarsening \n");
      printf("  -cljpci               : CLJP-c improved coarsening \n");
      printf("  -pmis                 : PMIS coarsening \n");
      printf("  -pmisc1               : PMIS-c coarsening (distance one coloring) \n");
      printf("  -pmisc2               : PMIS-c coarsening (distance two coloring) \n");
      printf("  -hmis                 : HMIS coarsening \n");
      printf("  -cr                   : CR coarsening \n");
      printf("  -cr-cljp              : CR coarsening (CLJP indep. set) \n");
      printf("  -cr-cljpc             : CR coarsening (CLJP-c indep. set) \n");
      printf("  -cr-pmis              : CR coarsening (PMIS indep. set) \n");
      printf("  -cr-pmisc1            : CR coarsening (PMIS-c1 indep. set) \n");
      ...
      printf("  -mxits  <val>          : maximum number of iterations (AMG)\n");
   ...
   else if(build_matrix_type == 6)
   {
     ...
     parcsr_A = (HYPRE_ParCSRMatrix) prob->A;

     /* Print total number of rows and number of rows on most
        heavily loaded processor. */
     int max_local_rows, min_local_rows;
     MPI_Allreduce(&hypre_CSRMatrixNumRows(hypre_ParCSRMatrixDiag((hypre_ParCSRMatrix*)parcsr_A)), &max_local_rows, 1, MPI_INT, MPI_MAX, comm);
     MPI_Allreduce(&hypre_CSRMatrixNumRows(hypre_ParCSRMatrixDiag((hypre_ParCSRMatrix*)parcsr_A)), &min_local_rows, 1, MPI_INT, MPI_MIN, comm);
     if(myid == 0)
     {
       int total_rows = hypre_ParCSRMatrixGlobalNumRows((hypre_ParCSRMatrix *)parcsr_A);
       printf("Total rows: %d\nEven split: %d\nMax rows (single proc.): %d\nMin rows (single proc.): %d\n", total_rows, total_rows/num_procs, max_local_rows, min_local_rows);
     }
   }
   else
   {
...
/* Initial guess */
   ...
   else if ( build_rhs_type == 6 )
   {
      ...
      {
        printf("  RHS vector is 0\n");
        printf("  Initial guess has random components in 0 - 1\n");
      }
...
/* note: log is written to standard output, not to file */
      ...
      HYPRE_BoomerAMGSetMaxIter(amg_solver, max_iters);
   ...
   if (solver_id == 101)
     /* Author: David Alber
      * This is a special case of solver_id == 0 (AMG) that calls AMG a number
      * of time to test the same problem using the same coarsening algorithm.
      * Doing it this way saves time in matrix creation because the matrix is
      * created using Tzanio Kolev's aFEM library (called higher up in this
      * function.
      *
      * NOT WORKING! Hangs after the first iteration in the loop.
      ****/
   {
      // CLJP: 0, CLJP-c: 20, PMIS-c1: 21 PMIS-c2: 22, Falgout: 6,
      // PMIS: 8, HMIS: 10, CR: 30
      int cg_algs_len = 5;
      int cg_algs[] = {6, 20, 30, 8, 0};
      //int cg_algs_len = 8;
      //int cg_algs[] = {6, 20, 0, 30, 8, 10, 21, 22};
      
      HYPRE_IJVector      ij_b_orig;
      HYPRE_IJVector      ij_x_orig;

      hypre_ParVector    *b_orig;
      hypre_ParVector    *x_orig;

      HYPRE_IJVectorCreate(MPI_COMM_WORLD, first_local_row, last_local_row, &ij_b_orig);
      HYPRE_IJVectorSetObjectType(ij_b_orig, HYPRE_PARCSR);
      HYPRE_IJVectorInitialize(ij_b_orig);
      ierr = HYPRE_IJVectorGetObject( ij_b_orig, &object );
      b_orig = (hypre_ParVector *) object;

      /* Initial guess */
      HYPRE_IJVectorCreate(MPI_COMM_WORLD, first_local_col, last_local_col, &ij_x_orig);
      HYPRE_IJVectorSetObjectType(ij_x_orig, HYPRE_PARCSR);
      HYPRE_IJVectorInitialize(ij_x_orig);
      ierr = HYPRE_IJVectorGetObject( ij_x_orig, &object );
      x_orig = (hypre_ParVector *) object;

      hypre_ParVectorCopy((hypre_ParVector *)b, b_orig);
      hypre_ParVectorCopy((hypre_ParVector *)x, x_orig);
      

      for(i = 0; i < cg_algs_len; i++) {
	hypre_ParVectorCopy(b_orig, (hypre_ParVector *)b);
	hypre_ParVectorCopy(x_orig, (hypre_ParVector *)x);
	coarsen_type = cg_algs[i];
	if (myid == 0) printf("Solver:  AMG\n");
	time_index = hypre_InitializeTiming("BoomerAMG Setup");
	hypre_BeginTiming(time_index);
