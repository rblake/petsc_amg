 ...
 * Revision:      @(#) $Id: sidl_DFinder_Stub.c,v 1.4 2005/11/14 21:20:24 painter Exp $
...
sidl_DFinder
sidl_DFinder__create()
{
  return (*(_getExternals()->createObject))();
}

static sidl_DFinder sidl_DFinder__remote(const char* url,
...
{
  return sidl_DFinder__remote(url, _ex);
}
...
{
  return sidl_DFinder__remoteConnect(url, _ex);
}
...
const char* name)
{
  ...
  else if(sidl_DFinder_isType(self, name)) {
...
remote_sidl_DFinder_addRef(
  /* in */ struct sidl_DFinder__object* self /* TLD */)
{
...
static void sidl_DFinder__init_remote_epv(void)
{
  ...
  epv->f_queryInt           = remote_sidl_DFinder_queryInt;
  ...
  e2->f_setSearchPath = (void (*)(void*,const char*)) epv->f_setSearchPath;
  e2->f_getSearchPath = (char* (*)(void*)) epv->f_getSearchPath;
  e2->f_addSearchPath = (void (*)(void*,const char*)) epv->f_addSearchPath;
...
static struct sidl_DFinder__object*
sidl_DFinder__remoteConnect(const char *url, sidl_BaseInterface *_ex)
{
  struct sidl_DFinder__object* self;
...
static struct sidl_DFinder__object*
sidl_DFinder__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct sidl_DFinder__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "sidl.DFinder", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct sidl_DFinder__object*) malloc(
      sizeof(struct sidl_DFinder__object));
