 ...
 * Revision:      @(#) $Id: sidl_SIDLException_Module.c,v 1.4 2005/11/14 21:20:15 painter Exp $
...
static int
sidl_SIDLException_createCast(PyObject *self, PyObject *args,                 \
  PyObject *kwds) {
  ...
  int _okay = PyArg_ParseTupleAndKeywords(args, kwds, "|O&", _kwlist,         \
    (void *)sidl_SIDLException__convert, &optarg);
  if (_okay) {
    if (!optarg) {
...
{
  struct sidl_SIDLException__object *sidlobj;
...
messages and stack traces."
  ...
  PyTuple_SetItem(_sidl_SIDLExceptionType.tp_bases,0,                         \
    (PyObject *)sidl_BaseClass_PyType());
  PyTuple_SetItem(_sidl_SIDLExceptionType.tp_bases,1,                         \
