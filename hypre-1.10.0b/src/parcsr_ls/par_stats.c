                        ...
                        hypre_ParCSRMatrix *A         )
{
   ...
   if (my_id==0)
   {
	...
	printf(" Coarsening Type = CLJP, fixed random \n");
      ...
      else if (abs(coarsen_type) == 20) 
      {
	printf(" Coarsening Type = CLJP-c \n");
      }
      else if (abs(coarsen_type) == 120) 
      {
	printf(" Coarsening Type = BSIS \n");
      }
      else if (abs(coarsen_type) == 21) 
      {
	printf(" Coarsening Type = PMIS-c1 \n");
      }
      else if (abs(coarsen_type) == 22) 
      {
	printf(" Coarsening Type = PMIS-c2 \n");
      }
      else if (abs(coarsen_type) == 30)
      {
	printf(" Coarsening Type = Compatible Relaxation (CLJP-c Indep. Set)\n");
      }
      else if (abs(coarsen_type) == 31)
      {
	printf(" Coarsening Type = Compatible Relaxation (PMIS-c1 Indep. Set)\n");
      }
      else if (abs(coarsen_type) == 32)
      {
	printf(" Coarsening Type = Compatible Relaxation (CLJP Indep. Set)\n");
      }
      else if (abs(coarsen_type) == 33)
      {
	printf(" Coarsening Type = Compatible Relaxation (PMIS Indep. Set)\n");
      }
      if (coarsen_type > 0) 
      {
