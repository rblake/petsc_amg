                   ...
                   hypre_ParVector    *u         )
{
   ...
   int      local_offd_coeffs;
   int      max_offd_coeffs;
   int      total_offd_coeffs;
   int      total_fine_offd_coeffs;
   double  *num_coeffs;
...
*/
   ...
   for (j=0;j<hypre_ParAMGDataNumLevels(amg_data);j++)
   {
      ...
      offd = hypre_ParCSRMatrixOffd(A_array[j]);
      local_offd_coeffs += hypre_CSRMatrixNumCols(offd);
   }
   MPI_Allreduce(&local_offd_coeffs, &max_offd_coeffs, 1, MPI_INT, MPI_MAX, comm);
   MPI_Allreduce(&local_offd_coeffs, &total_offd_coeffs, 1, MPI_INT, MPI_SUM, comm);

   offd = hypre_ParCSRMatrixOffd(A_array[0]);
   MPI_Allreduce(&hypre_CSRMatrixNumCols(offd), &total_fine_offd_coeffs, 1, MPI_INT, MPI_SUM, comm);
   if(total_fine_offd_coeffs == 0)
     total_fine_offd_coeffs = 1;
   ...
   if (my_id == 0 && amg_print_level > 1)
   {
      ...
      printf("\n\n     Complexity:    grid = %f\n",grid_cmplxty);
      printf("                operator = %f\n",operat_cmplxty);
      printf("                    comm = %f\n", ((double)(total_offd_coeffs)) / total_fine_offd_coeffs);
      printf("               comm_mean = %f\n", ((double)(total_offd_coeffs)) / num_procs);
      printf("            comm_max_raw = %f\n", (double)(max_offd_coeffs));
      printf("                   cycle = %f\n\n\n\n",cycle_cmplxty);
   }
